...
# 仍然加载数据文件（可选）
try:
    data = pd.read_csv(os.path.join(script_dir, "aapl_sp500_volatility.csv"))
    fed_data = pd.read_csv(os.path.join(script_dir, "fed_funds_rate.csv"))
    sentiment_data = pd.read_excel(os.path.join(script_dir, "news_sentiment_data.xlsx"))
except:
    st.info("🔍 某些数据文件未加载成功，这不会影响模型预测。")
...
st.markdown("这个应用允许你输入宏观和情绪因子，用于预测苹果未来一周平均股价。")
